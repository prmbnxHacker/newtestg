...
def process_master_pdf(user_pdf_path, output_path, original_filename, ai_percentage, shared_id):
    ...
    if has_noto:
        try:
            page1.insert_font(fontname="notosans", fontfile=notosans_path)
            page1.insert_font(fontname="notosans_sb", fontfile=notosans_semibold_path)
        except:
            has_noto = False
    ...
    for old_txt, new_txt in replacements.items():
        ...
        for inst in page1.search_for(old_txt):
            rect_to_clear = fitz.Rect(inst.x0 - 38, inst.y0 - 1, inst.x1 + 8, inst.y1 + 1) if old_txt in [old_pages_text, old_words_text, old_chars_text] else inst
            ...
            if has_noto and is_main_title:
                page1.insert_text((x_pos, inst.y1 - 1), str(new_txt), fontsize=19, fontname="notosans_sb", color=(0, 0, 0))
            elif has_noto:
                page1.insert_text((x_pos, inst.y1 - 1), str(new_txt), fontsize=9.5, fontname="notosans", color=(0, 0, 0))
            else:
                page1.insert_text((x_pos, inst.y1 - 1), str(new_txt), fontsize=19 if is_main_title else 9.5, fontname="helv", color=(0, 0, 0))
    ...
    if aa_matches:
        for aa_inst in aa_matches:
            ...
            page1.apply_redactions()
            if has_noto:
                page1.insert_text((aa_inst.x0, aa_inst.y1), "Labib Hasan", fontsize=20, fontname="notosans_sb", color=(0, 0, 0))
            else:
                page1.insert_text((aa_inst.x0, aa_inst.y1), "Labib Hasan", fontsize=20, fontname="helv", color=(0, 0, 0))
    ...
    if len(template_doc) > 1:
        ...
        if group_inst:
            ...
            page2.insert_text((x_pos, group_inst[0].y1 - 1.5), str(random_detection_num), fontsize=8.5, fontname="helv", color=(0, 0, 0))
